...
class BETAn():
    def __init__(self):
        ...
        self.T = 87.0 #K
        self.Argon_cs = 0.142e-18 #m2/molecule Ref: Snurr2010
        
        self.loadunits = 'mol/kg'
        self.weight_of_box = 1e-20 #gm
        
        self.minlinelength = 4 #min number of points for it to be called a line.
        
        self.R2cutoff = 0.9995 #The way this works is if a region satisfies this crierion while satisfying other conditions,
        self.R2min = 0.998 #This is the minimum R2 value required for a region to be considered a line. 
    ...
    def makeisothermplot(self, ax, data, yerr=None, maketitle='Yes', tryminorticks = "Yes", xscale='log', with_fit="No", fit_data=None):
        '''
    ...
    def makeconsistencyplot(self, ax3, data, maketitle='Yes', tryminorticks = "Yes"):
        '''
    ...
    def makelinregplot(self, ax2, p,q, data, maketitle='Yes', mode='BET'):
        '''
    ...
    def eswdata(self, data, eswpoints = 3):
        ...
        #print (str(minima))
        if minima!=None:
            eswarea = data[data.index==minima]['Loading'].values[0]/1000 * self.N_A * self.Argon_cs #m2/g
        else:
            eswarea = None
        return [data['Loading'], data['phi'], minima , eswarea, []]

    def makeeswplot(self,ax, data, eswpoints=3, maketitle='Yes', with_fit='No', fit_data=None):
        '''
...
#            fig2=False
    ...
    def picklen(self,data, method='BET-ESW'):
        ...
        to the smallest. We will asign prioroty in the following order:
        ...
        for i in np.arange(end, endlowlimit, -1):
            for j in np.arange(start , i-minlength,1):
                ...
                if q - p > 10:
                    ...
                    linear region length so that we dont end up selecting a really huge linear region.  
                    '''
                    continue
                [linear ,stats, C, qm, x_max,x_BET3,x_BET4, con1,con2,con3,con4,A_BET, makefig2] = self.linregauto(p,q,data)
                [ftest, ttest,outlierdata, shaptest, r2, r2adj, results]=stats
    ...
    def saveimgsummary(self, bet_info, betesw_info, data, name ='', sumpath=os.path.join(os.curdir,'imgsummary'),saveindividual = "No",path=os.path.join(os.curdir, 'charts'),  dpi=300, eswminima = None ):
        '''
        ...
        if saveindividual=="Yes":
            ...
            self.makeisothermplot(ax,data, maketitle='No', with_fit='Yes', fit_data=[bet_info, betesw_info])
            self.makeconsistencyplot(ax3, data, maketitle='No')
            self.makelinregplot(ax2, rbet[0],rbet[1],data, maketitle='No')
            self.makeeswplot(ax4,data,maketitle='No', with_fit='Yes', fit_data=[bet_info, betesw_info ])
            self.makelinregplot(ax5, rbetesw[0], rbetesw[1], data, maketitle="No")
            fig.savefig(os.path.join(path, 'Isotherm%s.jpg'%(name)),format='jpg', dpi = dpi, bbox_inches='tight')
            fig3.savefig( os.path.join(path, 'BETPlot2_%s.jpg'%(name) ), format='jpg', dpi = dpi, bbox_inches='tight')
            fig2.savefig( os.path.join( path , 'BETPlot%s.jpg'%(name) ), format ='jpg', dpi = dpi, bbox_inches = 'tight')
            fig4.savefig( os.path.join( path , 'ESWPlot%s.jpg'%(name) ), format ='jpg', dpi = dpi, bbox_inches = 'tight')
            fig5.savefig( os.path.join( path , 'BETESWPlot%s.jpg'%(name) ), format ='jpg', dpi = dpi, bbox_inches = 'tight')
            plt.close(fig)
            plt.close(fig3)
            plt.close(fig2)
            plt.close(fig4)
        ...
        self.makeisothermplot(axf, data, with_fit='Yes', fit_data=[bet_info, betesw_info])
        self.makeconsistencyplot(ax3f, data)
        self.makelinregplot(ax2f,rbet[0],rbet[1], data)
        self.makeeswplot(ax4f,data,  with_fit='Yes', fit_data=[bet_info, betesw_info ])
        if eswminima==None:
            ax5f.axis('off')
        else:
            self.makelinregplot(ax5f, rbetesw[0], rbetesw[1], data, mode='BET-ESW')
        blanksubplot.axis('off')
        figf.tight_layout()
        figf.savefig(os.path.join( sumpath, '%sSummary.png'%name), format='png', dpi=dpi, bbox_inches='tight')
        plt.close(figf)
    ...
    def generatesummary(self, data, name='structure', filepath= os.curdir, filename='summary.txt', eswpoints=3, writetofile="Yes", makeimagesummary="Yes", 
                        sumpath=os.path.join(os.curdir, 'imgsummary'),saveindividual="No", path=os.path.join(os.curdir, 'charts'), df_path=os.curdir , dpi=300):
        '''
        ...
        #We write out the cleaned out the data to the output file so that people know the BET consistency 1 and 2 values that we have. Eventually, we will also modify 
        #the headers. 
        ...
        if makeimagesummary=="Yes":
            ...
            self.saveimgsummary( bet_info, betesw_info, data, name=name, sumpath=sumpath, saveindividual = saveindividual, path=path, dpi=dpi, eswminima = eswminima)
...
if __name__=="__main__":
    ...
    datapath = os.path.join(home,'priyadata','Data_Archit','isotherm')
    b= BETAn()
    
    #datafile = 'Arja809985t.dat'
    datafile = 'Arja809985t_si_002.dat'
    data = pd.read_table(os.path.join(datapath,datafile), usecols = [0,1], names = ['Pressure', 'Loading'], header = None)
    data = b.prepdata(b.cleandata(data))
    
    
    #outfilepath = os.path.join(home, 'priyadata', 'analysis/')
    #outfilename = 'summary200.txt'
    
    #b.run_standard(datapath, datafile, outfilepath,  clearoutfile="No", eswpoints=3, writetofile="No")
    
    '''
    #datapath = home +'priyadata/Files_handover/CNTs/isotherms/'
    datapath = home+ 'priyadata/Data_Archit/isotherm/'
    os.chdir(os.curdir)
    filelist = os.listdir(datapath)
    b= BETAn()
    b.prepoutfile('./', 'summary.csv')

    datalist =[]
    for i in range(len(filelist)):
        if fnmatch.fnmatch(filelist[i],'*'+'.dat'):
            datalist.append(filelist[i])
    datafile = 'ArTOVGAO.dat'
    b.run_standard(datapath, datafile, home+'priyadata/', errorfilepath=None, clearoutfile='No', 
                   outfilename='summary.txt', errorfilename='errorlog.txt', sep='\t', 
                   usecols=[0, 1], name=None, saveindividual='No', path='./', eswpoints=2, writetofile='No', 
                   makeimgsummary='Yes', sumpath=home+'priyadata/analysis/', dpi=300, na_values=None)
    #datafile = 'ArCNT-16-16_unitcell_LoadingE87.0.dat'
    #df = pd.read_csv(datapath+datafile, sep='\t', usecols = [0,1], names=['Pressure', 'Loading'],header=None, engine='python')
    #df = pd.read_csv(datapath+datafile, sep='    ', usecols = [0,2], names=['Pressure', 'Loading'],header=None, engine='python')
    #dataog = df
    #units = "mol/kg"
    #name = datafile[:-4]
    
    #data = b.prepdata(b.cleandata(dataog))
    #b.con1limit = b.getoptimum(data,column='BET_y2', x='P_rel', how='Maxima', which=0, points=2 )[0]
    #b.generatesummary(data, name, saveindividual="No",eswpoints=2,filepath ='./', filename='testsummary.csv',sumpath = './')
    '''
    ...
    df = pd.read_csv('./isodata/'+datafile, sep='     ', header=None, engine='python')
    dataog = df[[0,2]]
    dataog.columns = ['Pressure', 'Loading']
    units = "mol/kg"
    name = datafile[:-4]
