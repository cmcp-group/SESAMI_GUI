...
# from SESAMI_2.ML import ML # importing the ML class
...
class ML():
    ...
    def pressure_bin_features(self, data, pressure_bins, 
        ...
        # TODO fix code and rerun model from before
          # to include the 0,0 point
          # TODO this will help if any user uploaded isotherm has no points under 5 Pa

        print(f'pressure_bins are {pressure_bins}') # TODO remove later

        column_names = ['Pressure', 'Loading']
        df = pd.read_table('SESAMI_1/user_structure/user_isotherm.txt', skiprows=1, sep='\t', names=column_names) # That text file gets made by the GUI python script.
...
def calculation_v2_runner():
  ...
  output_data_path='SESAMI_2/Outputs' #Output data path. 
  ...
  lasso = pickle.load(open('SESAMI_2/lasso_model.sav', 'rb'))
